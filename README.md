...
```shell
docker build --build-arg VERSION=$(uvx --from setuptools-scm python -m setuptools_scm) -t boozebot .
```

2. Run the container, mounting your data directory:

```shell
docker run --rm --name boozebot -v ./ptn/data:/root/boozedatabase boozebot
```
